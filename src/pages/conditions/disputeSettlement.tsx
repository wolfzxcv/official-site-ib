    ...
    <Wrapper>
      ...
      <Flex my={10} direction="column" align="center">
        ...
        <Box width={'80vw'} my={2} textAlign={'left'}>
          ...
          <Stack spacing={5}>
            <Text>{`1. ${t('collectMaterials')}`}</Text>
            <Text>{`2. ${t('forCurrentTrades')}`}</Text>
            <Text>{`3. ${t('transferTo')}`}</Text>
            <Text>{`4. ${t('contactOurComplianceDepartment')}`}</Text>
            <Text>{`5. ${t('collectAllTheSupportingMaterials')}`}</Text>
          </Stack>
